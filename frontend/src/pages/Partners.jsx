...
export default function Partners() {
  ...
  const industries = useMemo(() =>
    data ? [...new Set(data.PARTNERS_WITH_META.map(p => p.industry))].sort() : [],
    [data]
  )

  const partners = useMemo(() => {
    if (!data) return []
    let rows = [...data.PARTNERS_WITH_META]
    ...
    return rows
  }, [data, search, filterSt, filterTier, filterInd, sortKey, sortDir])
  ...
  const counts = STATUSES.reduce((m, s) => {
    m[s] = data.PARTNERS_WITH_META.filter(p => p.status === s).length; return m
  }, {})
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-3">
        {STATUSES.map(s => (
          <button key={s} onClick={() => setFilterSt(f => f === s ? '' : s)}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="파트너명 또는 산업 검색…" value={search}
        ...
        <Sel value={filterTier} onChange={setFilterTier} placeholder="등급"   options={TIERS.map(t => ({ value: t, label: `Tier ${t}` }))} />
        <Sel value={filterInd}  onChange={setFilterInd}  placeholder="산업군" options={industries.map(i => ({ value: i, label: i }))} />
        {(filterSt || filterTier || filterInd || search) && (
          <button onClick={() => { setSearch(''); setFilterSt(''); setFilterTier(''); setFilterInd('') }}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-neutral-800 text-neutral-500 text-xs uppercase tracking-wider">
              ...
              <th className="text-left px-5 py-3 font-medium cursor-pointer hover:text-neutral-300 select-none" onClick={() => toggleSort('ltv')}>
                LTV <SortArrow k="ltv" />
              </th>
              <th className="text-left px-5 py-3 font-medium cursor-pointer hover:text-neutral-300 select-none" onClick={() => toggleSort('risk')}>
          ...
          <tbody>
            {partners.map(p => {
              ...
              const rCl  = rs === null ? 'text-neutral-600' : rs < 30 ? 'text-emerald-400' : rs < 60 ? 'text-amber-400' : 'text-red-400'
              const rBar = rs === null ? '#374151' : rs < 30 ? '#10b981' : rs < 60 ? '#f59e0b' : '#ef4444'
              return (
                <tr key={p.id} className="border-b border-neutral-800/40 hover:bg-neutral-800/30 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${STATUS_CLS[p.status]}`}>{STATUS_LABEL[p.status]}</span>
                  </td>
                  <td className="px-5 py-3">
                    {p.tier && <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${TIER_CLS[p.tier]}`}>Tier {p.tier}</span>}
                  </td>
                    ...
                    {rs !== null ? (
                      <div className="flex items-center gap-2">
                        <div className="w-12 h-1 bg-neutral-800 rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${rs}%`, background: rBar }} />
                        </div>
                        <span className={`text-xs font-mono ${rCl}`}>{rs}</span>
                      </div>
            ...
            {partners.length === 0 && (
              <tr><td colSpan={7} className="px-5 py-12 text-center text-neutral-600 text-sm">조건에 맞는 파트너가 없습니다.</td></tr>
            )}
          </tbody>
        </table>
      </div>
