...
import {
  ...
  LAST_12_MONTHS, TIER_DISTRIBUTION, TOP_PARTNERS_LTV,
  ACTIVITIES, PARTNERS,
...
export default function Dashboard() {
  ...
  const riskLevel = AVG_RISK < 30 ? { label: 'Low', color: 'text-emerald-400' }
                  : AVG_RISK < 60 ? { label: 'Medium', color: 'text-amber-400' }
                  :                  { label: 'High', color: 'text-red-400' }

  const kpis = [
    {
      icon: DollarSign, label: '이번달 매출', color: { bg: 'bg-indigo-500/15', text: 'text-indigo-400' },
      value: fmtAmt(THIS_MONTH_REVENUE), sub: '2026년 5월', trend: MOM_GROWTH,
    },
    {
      icon: Users, label: '활성 파트너', color: { bg: 'bg-emerald-500/15', text: 'text-emerald-400' },
      value: `${ACTIVE_PARTNERS}개사`, sub: `전체 ${TOTAL_PARTNERS}개사 중`,
    },
    {
      icon: TrendingUp, label: '평균 LTV', color: { bg: 'bg-blue-500/15', text: 'text-blue-400' },
      value: fmtFull(AVG_LTV), sub: '파트너당 누적 매출',
    },
    {
      icon: AlertTriangle, label: '평균 리스크', color: { bg: 'bg-amber-500/15', text: 'text-amber-400' },
      value: `${AVG_RISK}점`, sub: riskLevel.label,
    },
  ]
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={LAST_12_MONTHS} margin={{ left: -10 }}>
              ...
              <Area
                ...
                fill="url(#revGrad)"
                dot={false}
                activeDot={{ r: 5, fill: '#818cf8', stroke: '#1a1a2e', strokeWidth: 2 }}
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>
              ...
              <PieChart width={160} height={160}>
                ...
                >
                  {TIER_DISTRIBUTION.map((entry, i) => (
                    <Cell key={entry.tier} fill={TIER_PALETTE[i]} />
                  ))}
                </Pie>
                <Tooltip
                  contentStyle={{ ...TT_STYLE.contentStyle }}
                  formatter={(v, name) => [`${v}개사`, `Tier ${name}`]}
                />
              </PieChart>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(p.ltv / maxLtv) * 100}%`,
                        background: `linear-gradient(90deg, ${TIER_COLORS[p.tier]}, ${TIER_COLORS[p.tier]}88)`,
                      }}
                    />
                  </div>
                </div>
                <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold shrink-0 ${
                  p.tier === 'A' ? 'bg-indigo-500/20 text-indigo-300'
                : p.tier === 'B' ? 'bg-amber-500/20 text-amber-300'
              ...
              return (
                <div key={a.id} className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-neutral-600 mt-0.5">{partner?.name} · {ago < 24 ? `${ago}h 전` : `${Math.floor(ago/24)}d 전`}</p>
                  </div>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded ${meta.bg} ${meta.text} shrink-0`}>
                    {meta.label}
                  </span>
                </div>
              )
